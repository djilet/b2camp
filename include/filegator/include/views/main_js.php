...
$(document).ready(function() {
	...
	$('.clear-button').on('click', function(){
		$(this).parents().children().children().val('');
		$('.filter-field input').keyup();
	});
	...
	$('.filter-field input').keyup(function(event) {
		...
		timer = setTimeout(function() {
			...
			$('table.file-list tr').each(function(index) {
				...
				var contents = $(this).children('td.filename').children().html().toLowerCase();
	...
	// new folder
	$('a.new-folder').parent().click(function(){
	...
	$('.delete-selected').click(function(){
		...
		// notice if nothing is selected
		if (isSelected('delete_selected') == false) return false;
	...
	$(document).on('click','.filename span', function(){
		console.log('---');
		console.log($(this).attr('href'));
		console.log('---');
			$('#big_modal').html(' ');
			var output = '<div id="view-file"></div>';
			output += '<a class="close-reveal-modal"></a>';
			$('#big_modal').append(output);
			var url = $(this).attr('href');
			$('#view-file').html('<iframe style="width: 100%; height: 785px;" src="http://docs.google.com/gview?url='+ url +'&embedded=true"></iframe>');
	...
	$('body').on('click', '.action-info', function() {
		...
		// download sub-button
		if (data_type != 'dir'){
			output += '<button id="print-button" type="button" class="nice radius button"><?php echo lang::get("Print")?></button>';
		...
		$('#rename-button').click(function(){
			...
			var output = '<div class="modal-content"><h5><?php echo lang::get("Rename")?> '+itemt+':</h5><input type="text" name="new-name" id="new-name" value="'+data_name[0]+'" crypt="'+data_crypt+'" class="text ui-widget-content ui-corner-all" /></div>';
		...
		$('#edit-button').click(function(){
			...
			$.ajax({
				 type:"GET",
				 url: '<?php echo gatorconf::get('base_url')?>/?edit-load='+data_crypt,
...
function isSelected(action){
	if ($('td input[type="checkbox"]:checked').is(":empty") == false){
		...
		if(action == 'delete_selected')
			text = '<?php echo lang::get('delete_selected')?>';
		else if(action == 'copy_selected')
			text = '<?php echo lang::get('copy_selected')?>';
		else if(action == 'move_selected')
			text = '<?php echo lang::get('move_selected')?>';
		else
			text = '<?php echo lang::get('element_selected')?>';
				
		$('#modal').html(' ');
		$('#modal').append('<p class="lead">'+ text +'</p><hr />');
...
function showDirectoryTree(title, ajaxcall, post_action){
	...
	output += '<div class="text-search-box"><span class="search-img"></span><input type="text" id="text-search" class="inputtext" placeholder="<?php echo lang::get("Search and Filter")?>" />';
	output += '</div>';
