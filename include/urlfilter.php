...
class URLFilter extends LocalObject
{
	...
	{
		$result = $this->_GetAsArray("URL", $exclude);
	...
	{
		$result = $this->_GetAsArray("Form", $exclude);
	 ...
	 * @return array
	 */
	function _GetAsArray($type = "URL", $exclude = array())
	{
					...
					if (count($value) > 0)
					{
						foreach ($value as $k => $v)
						{
							if (is_array($v))
								...
								$result[] = array("Name" => $key."[".$k."]", "Value" => ($type == "Form" ? htmlspecialchars($v) : urlencode($v)));
						}
					}
				}
				else if (strlen($value) > 0)
				{
					$result[] = array("Name" => $key, "Value" => ($type == "Form" ? htmlspecialchars($value) : urlencode($value)));
				}
			}
		}
