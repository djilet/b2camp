...
var PROJECT_DIR = '<?php echo htmlspecialchars(addcslashes(PROJECT_DIR, "\r\n'\\"));?>';
var cookieExpires = <?php echo COOKIE_EXPIRE;?>*30;

var width = (window.innerWidth > 0) ? window.innerWidth : screen.width;
if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) || width <= 800) {
	var IS_MOBILE = true;
}
else{
	var IS_MOBILE = false;
...
<?php 
	...
	$user->LoadBySession();
?>
var USER_ID = "<?php echo $user->GetProperty("UserID");?>";
...
function createCKEditor(name, toolbarSet, width, height, params)
{
	...
	var toolbars = {standart: [['Source','-','Maximize','-','Templates' ], [ 'Cut','Copy','Paste','PasteText','PasteFromWord','-','Undo','Redo' ],[ 'Find','Replace','-','SelectAll'], [ 'Link','Unlink','Anchor' ], [ 'Image','Flash','Table','HorizontalRule','SpecialChar','Iframe' ], '/',
					...
					basic: [['Templates' ], ['Cut','Copy','-','Undo','Redo' ],[ 'Find','Replace'], [ 'Link','Unlink' ], [ 'Image','Table','HorizontalRule'], '/',
							['Format', '-', 'Bold','Italic','Underline','Strike','Subscript','Superscript','TextColor','BGColor'], [ 'NumberedList','BulletedList','-','Outdent','Indent','-','JustifyLeft','JustifyCenter','JustifyRight','JustifyBlock']
					]
	};	
 	cfg = new Object({
		contentsCss : [ '<?php echo PROJECT_PATH; ?>/fckconfig/fck_editorarea.css'],
		width: width || '100%',
		height : height || '400px',
	...
	for (i = 0; i < elm.form.elements.length; i++)
	{
		if (elm.form.elements[i].type == "checkbox" && elm.form.elements[i].name == name)
		{
			if(elm.checked){
				...
				$(elm.form.elements[i]).prop('checked', true).trigger('change');
			}else{
				$(elm.form.elements[i]).iCheck('uncheck');
				$(elm.form.elements[i]).prop('checked', false).trigger('change');
			}
		}
	}
}
...
function GetSimpleModal(header, content)
{
	...
	html +='			</div>';
	html +='			<div class="modal-body">'+(content ? content : '')+'</div>';
	html +='		</div>';
	html +='	</div>';
	html +='</div>';
	return $(html);
